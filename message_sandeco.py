...
class MessageSandeco:
    ...
    def extract_common_data(self):
        ...
        self.instance_id = self.data.get("instanceId")
        self.source = self.data.get("source")
        self.message_timestamp = self.data.get("messageTimestamp")
        self.message_type = self.data.get("messageType")
        self.sender = self.data.get("sender")  # Disponível apenas para grupos
        self.participant = self.key.get("participant")  # Número de quem enviou no grupo
    ...
    def extract_text_message(self):
        ...
        self.text_message = self.data["message"].get("conversation")

    def extract_audio_message(self):
        """Extrai dados de uma mensagem de áudio e define como atributos da classe."""
        audio_data = self.data["message"]["audioMessage"]
        self.audio_base64_bytes = self.data["message"].get("base64")
    ...
    def extract_image_message(self):
        ...
        image_data = self.data["message"]["imageMessage"]
    ...
    def extract_document_message(self):
        ...
        document_data = self.data["message"]["documentMessage"]
